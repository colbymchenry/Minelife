...
public class FarmerHandler {
    ...
    static {
        try {
            config = new MLConfig(new File(Minelife.getConfigDirectory(), "jobs"), "farmer");
    ...
    public static int getLevel(UUID playerID) {
        try {
            ResultSet result = ModJobs.db.query("SELECT * FROM farmer WHERE playerID='" + playerID.toString() + "'");
            if(result.next()) {
    ...
    public static boolean replant(EntityPlayerMP player) {
        ...
        return MathHelper.getRandomDoubleInRange(r, 0, 100) < chance;
    }
