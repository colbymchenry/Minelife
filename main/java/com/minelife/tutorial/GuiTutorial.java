    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
        ...
        if (sections.isEmpty() || (section != null && section.pages.isEmpty())) {
            fontRenderer.drawString(TextFormatting.BOLD + "Wow... Such empty!",
                    ...
                    yPosition + ((bookHeight - fontRenderer.FONT_HEIGHT) / 2), 0);
        } else {
            // draw main menu
            if (section == null) {
                int sectionY = 0;
                for (Section section : sections) {
                    if (mouseX >= xPosition + 20 && mouseY >= yPosition + 16 + sectionY &&
                            mouseX <= xPosition + 20 + bookWidth - 20 && mouseY <= yPosition + 16 + sectionY + fontRenderer.FONT_HEIGHT) {
                        GuiHelper.drawDefaultBackground(xPosition + 15, yPosition + 12 + sectionY, bookWidth - 35, fontRenderer.FONT_HEIGHT + 6, 0xFFA219);
                    }

                    fontRenderer.drawString(section.name, xPosition + 20,
                ...
                for (String line : ((Page) section.pages.toArray()[pageIndex]).lines) {
                    if (line.startsWith("@image")) {
                        ...
                        fontRenderer.drawSplitString(StringHelper.ParseFormatting(line, '&'), xPosition + 18, yPosition + yOffset + 13, bookWidth - 30, 0xFFFFFF);
                        yOffset += (fontRenderer.FONT_HEIGHT * fontRenderer.listFormattedStringToWidth(StringHelper.ParseFormatting(line, '&'), bookWidth - 30).size());
                    }
                }
